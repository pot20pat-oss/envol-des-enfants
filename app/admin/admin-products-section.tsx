...
export function ProductsSection({ products, catalogProducts, market, busy, search, setSearch, category, setCategory, visibility, setVisibility, stock, setStock, synchronize, add, edit, adjustStock, remove, reload }: {
  ...
  const duplicatePairKey=(items:Row[])=>items.map(p=>String(p.id)).sort().join("|");
  const markDuplicate=(items:Row[],verdict:"confirmed"|"rejected"|null)=>{
    const key=duplicatePairKey(items);
    setDuplicateVerdicts(previous=>{
      const next={...previous};if(verdict)next[key]=verdict;else delete next[key];
      try{window.localStorage.setItem("cms-duplicate-verdicts-v1",JSON.stringify(next))}catch{}
      return next;
    });
  };
  ...
  const scanDuplicates = async () => {
    try{const saved=JSON.parse(window.localStorage.getItem("cms-duplicate-verdicts-v1")||"{}");if(saved&&typeof saved==="object"&&!Array.isArray(saved))setDuplicateVerdicts(saved)}catch{}
