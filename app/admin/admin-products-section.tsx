...
export function ProductsSection({ products, catalogProducts, market, busy, search, setSearch, category, setCategory, visibility, setVisibility, stock, setStock, synchronize, add, edit, adjustStock, remove, reload }: {
  ...
  const scanDuplicates = () => {
    ...
    try {
      ...
      const groups: Row[][]=[];const visited=new Set<string>();
      for(const id of adjacency.keys()){
        if(visited.has(id))continue;
        const stack=[id],group:Row[]=[];visited.add(id);
        while(stack.length){
          const current=stack.pop()!;const product=byId.get(current);if(product)group.push(product);
          for(const neighbor of adjacency.get(current)||[]){if(!visited.has(neighbor)){visited.add(neighbor);stack.push(neighbor)}}
        }
