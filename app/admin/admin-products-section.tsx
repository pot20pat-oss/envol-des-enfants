...
export function ProductsSection({ products, catalogProducts, market, busy, search, setSearch, category, setCategory, visibility, setVisibility, stock, setStock, synchronize, add, edit, adjustStock, remove, reload }: {
  ...
  const [duplicateScan, setDuplicateScan] = useState<{groups: Array<{products:Row[];visual:number;confidence:"certain"|"probable"|"review"}>; scanned: number} | null>(null);
  ...
  const scanDuplicates = async () => {
    ...
    try {
      ...
      const pairs:Array<{a:Row;b:Row;visual:number;match:number;confidence:"certain"|"probable"|"review"}>=[];const seen=new Set<string>();
