...
export function ProductsSection({ products, catalogProducts, market, busy, search, setSearch, category, setCategory, visibility, setVisibility, stock, setStock, synchronize, add, edit, adjustStock, remove, reload }: {
  ...
  return <section className="cms-panel">
    <div style={{position:"fixed",top:12,left:218,right:24,zIndex:9990,display:"flex",alignItems:"center",justifyContent:"space-between",gap:16,flexWrap:"nowrap",margin:0,padding:"10px 14px",border:"1px solid #dce6eb",borderRadius:12,background:"#fff",boxShadow:"0 6px 18px rgba(18,52,77,.14)"}}><strong style={{whiteSpace:"nowrap",color:"#123f61"}}>Produits</strong><input className="cms-search" placeholder="Nom, marque ou numéro d’article…" value={search} onChange={(event) => setSearch(event.target.value)} style={{flex:"1 1 360px",maxWidth:560,minWidth:220}} /><div style={{display:"flex",alignItems:"center",gap:10,marginLeft:"auto",whiteSpace:"nowrap"}}><button className="cms-secondary" disabled={busy} onClick={synchronize}>↻ Importer le catalogue</button><button className="cms-primary" onClick={add}>+ Ajouter manuellement</button></div></div>
