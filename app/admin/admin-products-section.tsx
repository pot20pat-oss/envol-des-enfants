...
export function ProductsSection({ products, catalogProducts, market, busy, search, setSearch, category, setCategory, visibility, setVisibility, stock, setStock, synchronize, add, edit, adjustStock, remove, reload }: {
  ...
  return <section className="cms-panel">
    ...
    {duplicateScan&&<section style={{margin:"0 0 18px",padding:16,border:"1px solid #b9cbd5",borderRadius:12,background:"#f8fbfc"}}><div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center"}}><strong>Scanner de doublons · {duplicateScan.scanned} produits vérifiés · {duplicateScan.groups.length} groupe(s) suspect(s)</strong><button className="cms-secondary" onClick={()=>setDuplicateScan(null)}>Fermer</button></div>{duplicateScan.groups.length===0?<p>Aucun doublon potentiel détecté avec les critères actuels.</p>:<div style={{display:"grid",gap:10,marginTop:12}}>{duplicateScan.groups.map((group,index)=><div key={index} style={{display:"grid",gap:12,padding:14,border:"1px solid #dce6eb",borderRadius:12,background:"#fff"}}><b>Doublon potentiel</b><div style={{display:"flex",gap:14,flexWrap:"wrap"}}>{group.map(product=><button key={String(product.id)} type="button" onClick={()=>edit(product)} style={{width:220,padding:10,border:"1px solid #cbd9df",borderRadius:10,background:"#fff",textAlign:"left",cursor:"pointer"}}>{product.image_url?<img src={String(product.image_url)} alt={String(product.name_fr||"Produit")} style={{display:"block",width:"100%",height:180,objectFit:"contain",background:"#fff",borderRadius:8,marginBottom:8}}/>:<div style={{height:180,display:"grid",placeItems:"center",background:"#f3f6f7",borderRadius:8,marginBottom:8}}>Aucune image</div>}<strong style={{display:"block"}}>{String(product.name_fr||product.article_number||"Produit")}</strong><small>{String(product.brand||"")} · No {String(product.article_number||"—")}</small></button>)}</div></div>)}</div>}</section>}
