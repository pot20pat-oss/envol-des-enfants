...
export function ProductsSection({ products, catalogProducts, market, busy, search, setSearch, category, setCategory, visibility, setVisibility, stock, setStock, synchronize, add, edit, adjustStock, remove, reload }: {
  ...
  const scanDuplicates = async () => {
    ...
    try {
      ...
      const pairs:Array<{a:Row;b:Row;visual:number;confidence:"certain"|"probable"|"review"}>=[];const seen=new Set<string>();
      ...
      groups.sort((a,b)=>{const bucket=(x:{visual:number})=>x.visual>=.975?0:x.visual>=.95?1:2;return bucket(a)-bucket(b)||b.visual-a.visual||rank[a.confidence]-rank[b.confidence]});
