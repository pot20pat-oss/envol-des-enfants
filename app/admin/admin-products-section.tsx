  ...
  const scanDuplicates = async () => {
    ...
    try {
      ...
      for(let i=0;i<source.length;i++)for(let j=i+1;j<source.length;j++){
        ...
        const metadata=Math.max(name*.55+distinctive*.30+(sameBrand?.15:0),sameBrand&&desc>=.72?Math.min(1,name*.65+desc*.20+.15):0);const visualEvidence=Math.max(scores.cropped,scores.legacy*.94);const visualBoost=visual>=.995?.99:visual>=.99?.97:visual>=.985?.94:visual>=.975?.90:0;const nameSupport=name>=.92?.03:name>=.80?.02:name>=.55?.01:0;const agreementSupport=Math.min(scores.legacy,scores.cropped)>=.97?.02:0;const match=sameArticle||scores.exact?1:Math.max(visualBoost,Math.min(.99,visualEvidence*.82+metadata*.18+nameSupport+agreementSupport));const legacyStrong=scores.legacy>=.975;const croppedStrong=scores.cropped>=.985;const metadataStrong=sameBrand||name>=.62||distinctive>=.55;const duplicate=sameArticle||scores.exact||semantic>=.90||legacyStrong||croppedStrong||(scores.cropped>=.955&&metadataStrong)||(sameBrand&&distinctive>=.72&&name>=.90&&desc>=.72);
        if(duplicate){const key=[aid,bid].sort().join("|");if(!seen.has(key)){seen.add(key);const strongNameVisual=scores.legacy>=.975&&name>=.92;const strongBrandVisual=sameBrand&&scores.legacy>=.965&&name>=.72;const strongCrop=scores.cropped>=.985&&metadata>=.48;const multiEvidence=strongNameVisual||strongBrandVisual||strongCrop;const probableEvidence=(scores.legacy>=.965&&name>=.72)||(scores.legacy>=.975&&metadata>=.12)||(scores.cropped>=.965&&metadata>=.32);const confidence:"certain"|"probable"|"review"=match>=.97||sameArticle||scores.exact||semantic>=.965||(semantic>=.93&&name>=.55)||multiEvidence?"certain":match>=.90||semantic>=.90||probableEvidence?"probable":"review";pairs.push({a,b,visual,match,semantic,legacy:scores.legacy,cropped:scores.cropped,name,distinctive,sameBrand,confidence})}}
      }
