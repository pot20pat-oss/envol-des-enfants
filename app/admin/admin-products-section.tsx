...
export function ProductsSection({ products, catalogProducts, market, busy, search, setSearch, category, setCategory, visibility, setVisibility, stock, setStock, synchronize, add, edit, adjustStock, remove, reload }: {
  ...
  return <section className="cms-panel">
    <div style={{position:"fixed",top:0,right:24,zIndex:9990,display:"flex",alignItems:"center",justifyContent:"space-between",gap:16,flexWrap:"wrap",margin:0,padding:"12px 16px",border:"1px solid #dce6eb",borderRadius:"12px 12px 10px 10px",background:"rgba(255,255,255,.98)",boxShadow:"0 8px 24px rgba(18,52,77,.16)",backdropFilter:"blur(8px)"}}><input className="cms-search" placeholder="Nom, marque ou numéro d’article…" value={search} onChange={(event) => setSearch(event.target.value)} style={{width:360,maxWidth:"36vw"}} /><div style={{display:"flex",alignItems:"center",gap:10,marginLeft:"auto"}}><button className="cms-secondary" disabled={busy} onClick={synchronize}>↻ Importer le catalogue</button><button className="cms-primary" onClick={add}>+ Ajouter manuellement</button></div></div>
