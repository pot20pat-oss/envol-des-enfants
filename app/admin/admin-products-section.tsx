...
export function ProductsSection({ products, catalogProducts, market, busy, search, setSearch, category, setCategory, visibility, setVisibility, stock, setStock, synchronize, add, edit, adjustStock, remove, reload }: {
  ...
  const [duplicateScan, setDuplicateScan] = useState<{groups: Array<{products:Row[];visual:number;match:number;legacy:number;cropped:number;name:number;distinctive:number;sameBrand:boolean;confidence:"certain"|"probable"|"review"}>; scanned: number} | null>(null);
  ...
  const scanDuplicates = async () => {
    ...
    try {
      ...
      const pairs:Array<{a:Row;b:Row;visual:number;match:number;legacy:number;cropped:number;name:number;distinctive:number;sameBrand:boolean;confidence:"certain"|"probable"|"review"}>=[];const seen=new Set<string>();
      for(let i=0;i<source.length;i++)for(let j=i+1;j<source.length;j++){
        ...
        const metadata=Math.max(name*.55+distinctive*.30+(sameBrand?.15:0),sameBrand&&desc>=.72?Math.min(1,name*.65+desc*.20+.15):0);const visualEvidence=Math.max(scores.cropped,scores.legacy*.94);const visualBoost=visual>=.99?.97:visual>=.985?.94:visual>=.975?.90:0;const match=sameArticle||scores.exact?1:Math.max(visualBoost,Math.min(1,visualEvidence*.82+metadata*.18));const legacyStrong=scores.legacy>=.975;const croppedStrong=scores.cropped>=.985;const metadataStrong=sameBrand||name>=.62||distinctive>=.55;const duplicate=sameArticle||scores.exact||legacyStrong||croppedStrong||(scores.cropped>=.955&&metadataStrong)||(sameBrand&&distinctive>=.72&&name>=.90&&desc>=.72);
        if(duplicate){const key=[aid,bid].sort().join("|");if(!seen.has(key)){seen.add(key);const multiEvidence=(visual>=.99&&metadata>=.28)||(scores.cropped>=.985&&metadata>=.48)||(scores.legacy>=.99&&metadata>=.52);const probableEvidence=(visual>=.977&&metadata>=.16)||(scores.cropped>=.975&&metadata>=.32)||(scores.legacy>=.982&&metadata>=.38);const confidence:"certain"|"probable"|"review"=sameArticle||scores.exact||multiEvidence?"certain":probableEvidence?"probable":"review";pairs.push({a,b,visual,match,legacy:scores.legacy,cropped:scores.cropped,name,distinctive,sameBrand,confidence})}}
      }
