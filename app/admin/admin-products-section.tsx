  ...
  const scanDuplicates = async () => {
    ...
    try {
      ...
      for(let i=0;i<source.length;i++)for(let j=i+1;j<source.length;j++){
        ...
        if(duplicate){const key=[aid,bid].sort().join("|");if(!seen.has(key)){seen.add(key);const multiEvidence=(visual>=.99&&metadata>=.20)||(scores.cropped>=.98&&metadata>=.42)||(scores.legacy>=.985&&metadata>=.45);const confidence:"certain"|"probable"|"review"=sameArticle||scores.exact||multiEvidence?"certain":(visual>=.975&&metadata>=.12)||(scores.cropped>=.965&&metadataStrong)?"probable":"review";pairs.push({a,b,visual,match,confidence})}}
      }
