  ...
  return <section className="cms-panel">
    ...
    <section style={{margin:"0 0 16px",padding:14,border:"1px solid #b9cbd5",borderRadius:12,background:"var(--cms-surface)",color:"var(--cms-text)"}}>
      <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
        ...
        <input ref={imageSearchInput} type="file" accept="image/*" disabled={imageSearchBusy} onChange={e=>{const file=e.target.files?.[0];if(file)void searchByImage(file)}} />
        {imageSearchBusy&&<span role="status">⏳ Comparaison avec le catalogue…</span>}
        {!!imageSearchResults.length&&<button type="button" className="cms-secondary" onClick={()=>setImageSearchResults([])}>Fermer les résultats</button>}
      </div>
      {!!imageSearchResults.length&&<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(150px,1fr))",gap:10,marginTop:12}}>
        {imageSearchResults.map(({product,score})=><button key={String(product.id)} type="button" onClick={()=>edit(product)} style={{padding:8,border:"1px solid #d7e2e7",borderRadius:10,background:"var(--cms-surface-raised)",color:"var(--cms-text)",textAlign:"left",cursor:"pointer"}}>
          {product.image_url&&<img src={String(product.image_url)} alt={String(product.name_fr||"Produit")} style={{width:"100%",height:130,objectFit:"contain",background:"#fff"}}/>}
          <strong style={{display:"block",fontSize:13,marginTop:6}}>{String(product.name_fr||product.article_number||"Produit")}</strong>
          <small>{Math.round(score*100)} % visuel · {String(product.article_number||"—")}</small>
        </button>)}
      </div>}
      {!imageSearchBusy&&imageSearchResults.length===0&&<small style={{display:"block",marginTop:6}}>Choisis une photo pour retrouver les produits visuellement similaires dans le catalogue.</small>}
    </section>
    {duplicateScan&&<section style={{margin:"0 0 18px",padding:16,border:"1px solid #b9cbd5",borderRadius:12,background:"#f8fbfc"}}><div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12,alignItems:"center"}}><strong>Test en 3 passes :</strong>{([1,2,3] as const).map(pass=><button key={pass} type="button" className={duplicatePass===pass?"cms-primary":"cms-secondary"} onClick={()=>setDuplicatePass(pass)}>{pass===1?"1 · Tous les suspects":pass===2?"2 · Preuves croisées":"3 · Priorité de vérification"} ({duplicateScan.groups.filter(g=>duplicateEligible(g,pass)).length})</button>)}<small>Vue active : passe {duplicatePass} · {visibleDuplicateGroups.length} paire(s) affichée(s) sur {duplicateScan.groups.length}. Les vrais doublons confirmés restent visibles en tête; les faux positifs sont masqués sans être supprimés. Le classement brut du scanner reste intact.</small><small>Validation manuelle : {duplicateScan.groups.filter(g=>duplicateVerdicts[duplicatePairKey(g.products)]==="confirmed").length} vrai(s) doublon(s) · {duplicateScan.groups.filter(g=>duplicateVerdicts[duplicatePairKey(g.products)]==="rejected").length} faux positif(s) · {duplicateScan.groups.filter(g=>duplicateVerdicts[duplicatePairKey(g.products)]==="variant").length} variante(s) distincte(s). Verdicts enregistrés dans D1.</small>{duplicateReviewStats&&<strong style={{fontSize:13}}>Révision : {duplicateReviewStats.unreviewed}/{duplicateReviewStats.total} à traiter · {duplicateReviewStats.confirmed} doublon(s) · {duplicateReviewStats.variant} variante(s) · {duplicateReviewStats.rejected} rejet(s)</strong>}{visibleDuplicateGroups.length>0&&<span style={{display:"inline-flex",gap:6,alignItems:"center"}}><button type="button" className="cms-secondary" disabled={safeDuplicateReviewIndex<=0} onClick={()=>setDuplicateReviewIndex(i=>Math.max(0,i-1))}>← Précédent</button><strong>{Math.min(safeDuplicateReviewIndex+1,visibleDuplicateGroups.length)}/{visibleDuplicateGroups.length}</strong><button type="button" className="cms-secondary" disabled={safeDuplicateReviewIndex>=visibleDuplicateGroups.length-1} onClick={()=>setDuplicateReviewIndex(i=>Math.min(visibleDuplicateGroups.length-1,i+1))}>Suivant →</button></span>}<label style={{display:"inline-flex",gap:6,alignItems:"center",fontSize:13}}><input type="checkbox" checked={duplicateReviewOnlyCrossBrand} onChange={e=>{setDuplicateReviewOnlyCrossBrand(e.target.checked);setDuplicateReviewOnlySameBrand(false);setDuplicateReviewIndex(0)}}/> Marques différentes</label><label style={{display:"inline-flex",gap:6,alignItems:"center",fontSize:13}}><input type="checkbox" checked={duplicateReviewOnlyDifferentArticles} onChange={e=>{setDuplicateReviewOnlyDifferentArticles(e.target.checked);setDuplicateReviewIndex(0)}}/> Références différentes</label><label style={{display:"inline-flex",gap:6,alignItems:"center",fontSize:13}}><input type="checkbox" checked={duplicateReviewOnlySameCategory} onChange={e=>{setDuplicateReviewOnlySameCategory(e.target.checked);setDuplicateReviewIndex(0)}}/> Même catégorie seulement</label><label style={{display:"inline-flex",gap:6,alignItems:"center",fontSize:13}}><input type="checkbox" checked={duplicateReviewOnlySameBrand} onChange={e=>{setDuplicateReviewOnlySameBrand(e.target.checked);if(e.target.checked)setDuplicateReviewOnlyCrossBrand(false);setDuplicateReviewIndex(0)}}/> Même marque seulement</label><button type="button" className="cms-secondary" onClick={resetDuplicateReviewFilters} disabled={duplicateReviewFilter==="unreviewed"&&!duplicateReviewSearch&& !duplicateReviewOnlySameBrand&&!duplicateReviewOnlySameCategory}>Réinitialiser les filtres</button><input aria-label="Rechercher dans les doublons" placeholder="Nom, marque, no article, catégorie…" value={duplicateReviewSearch} onChange={e=>setDuplicateReviewSearch(e.target.value)} style={{minWidth:260}}/><select aria-label="Filtrer les décisions de doublons" value={duplicateReviewFilter} onChange={e=>setDuplicateReviewFilter(e.target.value as typeof duplicateReviewFilter)}><option value="unreviewed">À réviser</option><option value="all">Toutes les décisions</option><option value="confirmed">Vrais doublons</option><option value="variant">Variantes distinctes</option><option value="rejected">Faux positifs</option></select><button type="button" className="cms-secondary" onClick={()=>setShowRejectedDuplicates(v=>!v)}>{showRejectedDuplicates?"Masquer les décisions masquées":"Afficher les décisions masquées"}</button></div><div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",flexWrap:"wrap"}}><strong>Scanner de doublons · {duplicateScan.scanned} produits vérifiés · {duplicateScan.groups.filter(g=>g.confidence==="certain").length} correspondance(s) forte(s) · {duplicateScan.groups.filter(g=>g.confidence==="probable").length} probable(s) · {duplicateScan.groups.filter(g=>g.confidence==="review").length} à vérifier</strong><div style={{display:"flex",gap:8}}><button className="cms-secondary" onClick={()=>setShowReviewDuplicates(v=>!v)}>{showReviewDuplicates?"Masquer les À vérifier":`Afficher tous les suspects (${duplicateScan.groups.filter(g=>g.confidence==="review").length})`}</button><button className="cms-secondary" onClick={()=>setShowDuplicateDiagnostics(v=>!v)}>{showDuplicateDiagnostics?"Masquer diagnostic":"Afficher diagnostic"}</button><button className="cms-secondary" onClick={()=>setDuplicateScan(null)}>Fermer</button></div></div>{duplicateScan.groups.length===0?<p>Aucun doublon potentiel détecté avec les critères actuels.</p>:visibleDuplicateGroups.length===0?<div style={{marginTop:12,padding:14,border:"2px solid #f0b429",borderRadius:10,background:"#fffaf0"}}><strong>Aucune paire ne satisfait les critères de cette vue avec les options actuelles.</strong><div style={{marginTop:8}}><button className="cms-secondary" onClick={()=>setShowReviewDuplicates(true)}>Afficher aussi les paires « À vérifier »</button></div></div>:<div style={{display:"grid",gap:10,marginTop:12}}>{visibleDuplicateGroups.map((result,index)=><div key={index} ref={result===activeDuplicateGroup?el=>el?.scrollIntoView({block:"nearest"}):undefined} style={{display:"grid",gridTemplateColumns:"140px minmax(0,1fr)",gap:12,padding:14,border:"1px solid #dce6eb",borderRadius:12,background:"var(--cms-surface)",color:"var(--cms-text)"}}><b style={{paddingTop:8}}>{result.confidence==="certain"?"🔴 Quasi certain":result.confidence==="probable"?"🟠 Très probable":"🟡 À vérifier"}<small style={{display:"block",marginTop:5,fontWeight:700}}>{Math.round(result.match*1000)/10}% confiance<small style={{display:"block",fontWeight:600,color:"#667b87"}}>{Math.round(result.visual*1000)/10}% visuel</small>{showDuplicateDiagnostics&&<small style={{display:"block",marginTop:7,fontWeight:600,lineHeight:1.45,color:"#4d6470"}}>Sémantique {result.semantic?`${Math.round(result.semantic*1000)/10}%`:"—"}<br/>Ancien {Math.round(result.legacy*1000)/10}%<br/>Recadré {Math.round(result.cropped*1000)/10}%<br/>Nom {Math.round(result.name*100)}%<br/>Distinctif {Math.round(result.distinctive*100)}%<br/>Marque {result.sameBrand?"✓":"✗"}</small>}</small></b><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,220px))",gap:12}}>{result.products.map(product=><button key={String(product.id)} type="button" onClick={()=>edit(product)} style={{display:"grid",gridTemplateRows:"190px auto",padding:8,border:"2px solid #cbd9df",borderRadius:10,background:"var(--cms-surface-raised)",color:"var(--cms-text)",textAlign:"left",cursor:"pointer",overflow:"hidden"}}><div style={{width:"100%",height:190,display:"grid",placeItems:"center",background:"#fff",borderRadius:7,overflow:"hidden",padding:8,boxSizing:"border-box"}}>{product.image_url?<img src={String(product.image_url)} alt={String(product.name_fr||"Produit")} loading="lazy" style={{display:"block",maxWidth:"100%",maxHeight:"100%",width:"auto",height:"auto",objectFit:"contain",objectPosition:"center"}}/>:<span style={{fontWeight:800,color:"#7b8990"}}>AUCUNE IMAGE</span>}</div><div style={{padding:"8px 2px 2px"}}><strong style={{display:"block",fontSize:13,lineHeight:1.25}}>{String(product.name_fr||product.article_number||"Produit")}</strong><small style={{display:"block",marginTop:4}}>{String(product.brand||"")} · No {String(product.article_number||"—")}</small></div></button>)} </div><div style={{gridColumn:"2",fontSize:12,color:"#516571",lineHeight:1.5}}>{(()=>{const depth=sharedHierarchyDepth(result.products[0],result.products[1]);const h0=productHierarchy(result.products[0]),h1=productHierarchy(result.products[1]);const differentVariant=!!(h0.variant&&h1.variant&&h0.variant!==h1.variant);const relation=depth===0?"Catégories différentes":depth===1?"Même catégorie, marques différentes":depth===2?"Même catégorie et marque":depth===3?"Même sous-groupe":differentVariant?"Même famille proposée, références différentes":"Même famille proposée";return <><strong>Classement indicatif :</strong> {result.products.map(product=>{const h=productHierarchy(product);return `${h.category||"Sans catégorie"} › ${h.group||"Sans marque"} › ${h.subgroup||"Famille inconnue"} › ${h.variant||"Sans référence"}`}).join("  |  ")}. <strong>{relation}.</strong>{differentVariant&&<span> Vérifier s’il s’agit de variantes distinctes avant toute suppression.</span>} Le classement ne confirme jamais un doublon.</>})()}</div><div style={{gridColumn:"2",display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>{savingDuplicatePairs.has(duplicatePairKey(result.products))&&<small role="status">Enregistrement du verdict…</small>}<button type="button" className="cms-secondary" aria-pressed={duplicateVerdicts[duplicatePairKey(result.products)]==="confirmed"} disabled={savingDuplicatePairs.has(duplicatePairKey(result.products))} onClick={()=>markDuplicate(result.products,"confirmed")}>✓ Vrai doublon</button><button type="button" className="cms-secondary" aria-pressed={duplicateVerdicts[duplicatePairKey(result.products)]==="rejected"} disabled={savingDuplicatePairs.has(duplicatePairKey(result.products))} onClick={()=>markDuplicate(result.products,"rejected")}>✕ Pas un doublon</button><button type="button" className="cms-secondary" aria-pressed={duplicateVerdicts[duplicatePairKey(result.products)]==="variant"} disabled={savingDuplicatePairs.has(duplicatePairKey(result.products))} onClick={()=>markDuplicate(result.products,"variant")}>◇ Variante distincte · garder les deux</button>{duplicateVerdicts[duplicatePairKey(result.products)]&&<><strong>Décision : {duplicateVerdictLabel(duplicateVerdicts[duplicatePairKey(result.products)])}</strong><button type="button" className="cms-secondary" disabled={savingDuplicatePairs.has(duplicatePairKey(result.products))} onClick={()=>markDuplicate(result.products,null)}>Annuler le verdict</button></>}</div><div style={{gridColumn:"2",display:"flex",gap:8,flexWrap:"wrap"}}>{result.products.map(product=><button key={`delete-${String(product.id)}`} type="button" className="cms-danger" onClick={()=>{if(!window.confirm(`Supprimer définitivement « ${String(product.name_fr||product.article_number||"ce produit")} » ?`))return;remove(String(product.id));setDuplicateScan(current=>current?{...current,groups:current.groups.filter(g=>!g.products.some(p=>String(p.id)===String(product.id)))}:current)}}>🗑 Supprimer {String(product.article_number||"article")}</button>)}</div></div>)}</div>}</section>}
    <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",padding:"10px 14px",marginBottom:10,border:"1px solid #d7e2e7",borderRadius:12,background:"var(--cms-surface)",color:"var(--cms-text)"}}><strong>Historique</strong><button type="button" className="cms-secondary" disabled={bulkBusy||undoStack.length===0} onClick={()=>void undoLast()}>↶ Annuler{undoStack.length?` · ${undoStack[undoStack.length-1].label}`:""}</button><button type="button" className="cms-secondary" disabled={bulkBusy||redoStack.length===0} onClick={()=>void redoLast()}>↷ Rétablir</button><span style={{color:"#667b87",fontSize:12}}>10 dernières actions en lot conservées pendant cette session</span></div><div className={`cms-bulk-toolbar${selectedProducts.size?" has-selection":""}`}><strong>Actions en lot · {selectedProducts.size} sélectionné(s)</strong><button type="button" className="cms-secondary" onClick={()=>setSelectedProducts(new Set(products.map(p=>String(p.id))))}>Tout sélectionner ({products.length})</button><button type="button" className="cms-secondary" disabled={!selectedProducts.size} onClick={()=>setSelectedProducts(new Set())}>Désélectionner</button><select value={bulkVisibility} onChange={e=>setBulkVisibility(e.target.value)}><option value="keep">Visibilité — ne pas modifier</option><option value="hidden">Masqué partout</option><option value="qc">Québec seulement</option><option value="conakry">Conakry seulement</option><option value="both">Québec + Conakry</option></select><select aria-label="Déplacer la sélection vers une boutique" value={bulkMarket} onChange={e=>setBulkMarket(e.target.value as typeof bulkMarket)}><option value="keep">Déplacer — ne pas modifier</option><option value="qc">→ Québec seulement</option><option value="conakry">→ Conakry seulement</option><option value="both">→ Québec + Conakry</option><option value="hidden">→ Retirer des boutiques</option></select><select value={bulkCategory} onChange={e=>setBulkCategory(e.target.value)}><option value="keep">Catégorie — ne pas modifier</option>{Object.entries(categories).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select><select value={bulkPriceMode} onChange={e=>setBulkPriceMode(e.target.value as "set"|"percent")}><option value="set">Prix fixe · {markets[market].label}</option><option value="percent">Variation % · {markets[market].label}</option></select><input type="number" step="0.01" placeholder={bulkPriceMode==="percent"?"Ex. 10 ou -5":"Nouveau prix"} value={bulkPrice} onChange={e=>setBulkPrice(e.target.value)} style={{width:130}}/><button type="button" className="cms-primary" disabled={bulkBusy||!selectedProducts.size} onClick={()=>void applyBulk()}>{bulkBusy?"⌛ Modification…":"Appliquer à la sélection"}</button></div>
