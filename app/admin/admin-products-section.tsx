...
export function ProductsSection({ products, catalogProducts, market, busy, search, setSearch, category, setCategory, visibility, setVisibility, stock, setStock, synchronize, add, edit, adjustStock, remove, reload }: {
  ...
  return <section className="cms-panel">
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:16,flexWrap:"wrap",marginBottom:18,padding:"14px 16px",border:"1px solid #dce6eb",borderRadius:12,background:"#fff"}}><input className="cms-search" placeholder="Nom, marque ou numéro d’article…" value={search} onChange={(event) => setSearch(event.target.value)} style={{flex:"1 1 320px",maxWidth:520}} /><div style={{display:"flex",alignItems:"center",gap:10,marginLeft:"auto",position:"static"}}><button className="cms-secondary" disabled={busy} onClick={synchronize}>↻ Importer le catalogue</button><button className="cms-primary" onClick={add}>+ Ajouter manuellement</button></div></div>
