...
export function ProductsSection({ products, catalogProducts, market, busy, search, setSearch, category, setCategory, visibility, setVisibility, stock, setStock, synchronize, add, edit, adjustStock, remove, reload }: {
  ...
  const duplicateEligible=(g:NonNullable<typeof duplicateScan>["groups"][number],pass:1|2|3)=> {
    ...
    return (showRejectedDuplicates||(verdict!=="rejected"&&verdict!=="variant"))&&(verdict==="confirmed"||(passesDuplicate(g,pass)&&(showReviewDuplicates||g.confidence!=="review")));
  };
  const visibleDuplicateGroups=(duplicateScan?.groups.filter(g=>duplicateEligible(g,duplicatePass))||[]).sort((a,b)=>Number(duplicateVerdicts[duplicatePairKey(b.products)]==="confirmed")-Number(duplicateVerdicts[duplicatePairKey(a.products)]==="confirmed"));
