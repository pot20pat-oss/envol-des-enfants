  ...
  const scanDuplicates = async () => {
    ...
    try {
      ...
      for(let i=0;i<source.length;i++)for(let j=i+1;j<source.length;j++){
        ...
        if(duplicate){const key=[aid,bid].sort().join("|");if(!seen.has(key)){seen.add(key);const multiEvidence=(scores.cropped>=.975&&metadata>=.45)||(scores.legacy>=.98&&metadata>=.55);const confidence:"certain"|"probable"|"review"=sameArticle||scores.exact||multiEvidence?"certain":legacyStrong||(scores.cropped>=.965&&metadataStrong)?"probable":"review";pairs.push({a,b,visual,match,confidence})}}
      }
