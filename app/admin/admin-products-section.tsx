...
export function ProductsSection({ products, catalogProducts, market, busy, search, setSearch, category, setCategory, visibility, setVisibility, stock, setStock, synchronize, add, edit, adjustStock, remove, reload }: {
  ...
  const [duplicateVerdicts,setDuplicateVerdicts]=useState<Record<string,"confirmed"|"rejected">>({});
  ...
  const markDuplicate=async(items:Row[],verdict:"confirmed"|"rejected"|null)=>{
  ...
  const duplicateEligible=(g:NonNullable<typeof duplicateScan>["groups"][number],pass:1|2|3)=> {
    ...
    return (showRejectedDuplicates||verdict!=="rejected")&&(verdict==="confirmed"||(passesDuplicate(g,pass)&&(showReviewDuplicates||g.confidence!=="review")));
  };
