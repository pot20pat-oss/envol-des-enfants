...
export function OrdersSection({ orders, market, search, setSearch, status, setStatus, date, setDate, exportOrders, add, edit }: {
  ...
  return <section className="cms-panel">
    ...
    <div className="cms-table-wrap"><table><thead><tr><th>Date</th><th>Client</th><th>Panier / produits</th><th>Total</th><th>Statut</th><th></th></tr></thead><tbody>{orders.map((order) => <tr key={String(order.id)}><td>{new Date(String(order.created_at)).toLocaleDateString("fr-CA")}</td><td><strong>{String(order.customer_name)}</strong><small>{String(order.customer_phone)}</small>{order.customer_email&&<small>{String(order.customer_email)}</small>}{order.delivery_address&&<small>{String(order.delivery_address)}</small>}</td><td>{String(order.product_name)}{order.source==="storefront"&&<small>Commande envoyée depuis le panier client</small>}</td><td>{marketPrice(order.total, market)}</td><td><span className={`cms-status ${String(order.status)}`}>{orderLabels[String(order.status)]}</span></td><td><button className="cms-inline" onClick={() => edit(order)}>Modifier</button></td></tr>)}</tbody></table></div>
