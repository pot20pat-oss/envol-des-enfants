...
type Item={id:string;file:File;preview:string;hash:string;visualHash:string;duplicate:boolean;duplicateKind?:"exact"|"visual";url?:string;suggestion?:Row;group?:string;state:"ready"|"uploading"|"analyzing"|"done"|"error";error?:string};
...
export function AiBatchImport({market,busy,onDone}:{market:Market;busy:boolean;onDone:()=>Promise<void>}){
 const[items,setItems]=useState<Item[]>([]);const[working,setWorking]=useState(false);const unique=useMemo(()=>items.filter(i=>!i.duplicate),[items]);
 ...
 async function createAll(){setWorking(true);try{const done=items.filter(x=>!x.duplicate&&x.state==="done"&&x.suggestion&&x.url);const groups=new Map<string,Item[]>();for(const item of done){const key=item.group||item.id;groups.set(key,[...(groups.get(key)||[]),item])}for(const group of groups.values()){const first=group[0];const urls=group.map(x=>x.url!).filter(Boolean);await request("/api/admin/products",{method:"POST",body:JSON.stringify({...first.suggestion,image_url:urls[0],images_json:JSON.stringify(urls.slice(1)),price_qc:0,price_conakry:0,stock_qc:0,stock_conakry:0,visible_qc:market==="qc",visible_conakry:market==="conakry"})})}await onDone();setItems([])}finally{setWorking(false)}}
 ...
 return <section className="cms-ai-batch"><div className="cms-ai-batch-head"><div><h3>Importation intelligente par IA</h3><p>Détection des doublons exacts et visuels, regroupement des différentes vues d’un même produit, puis génération des fiches FR/EN.</p></div><label className="cms-primary cms-ai-file">Choisir des photos<input type="file" accept="image/*" multiple disabled={busy||working} onChange={e=>void choose(e.target.files)}/></label></div>{items.length>0&&<><div className="cms-ai-summary"><strong>{items.length}</strong> image(s) · <strong>{duplicates}</strong> doublon(s) éliminé(s) · <strong>{unique.length}</strong> à analyser · <strong>{groups}</strong> produit(s) regroupé(s) · <strong>{ready}</strong> image(s) prête(s)</div><div className="cms-ai-grid">{items.map(item=><article key={item.id} className={item.duplicate?"is-duplicate":""}><img src={item.preview} alt=""/><div><strong>{item.file.name}</strong>{item.duplicate?<span>{item.duplicateKind==="exact"?"Doublon exact":"Doublon visuel probable"} — ignoré</span>:item.suggestion?<><input value={String(item.suggestion.name_fr||"")} onChange={e=>update(item.id,"name_fr",e.target.value)}/><select value={String(item.suggestion.category||"eveil")} onChange={e=>update(item.id,"category",e.target.value)}>{Object.entries(categories).map(([v,l])=><option value={v} key={v}>{l}</option>)}</select><textarea value={String(item.suggestion.description_fr||"")} onChange={e=>update(item.id,"description_fr",e.target.value)}/><small>{item.group&&items.filter(x=>x.group===item.group).length>1?`Même produit : ${items.filter(x=>x.group===item.group).length} photos`:"Produit unique"} · EN : {String(item.suggestion.description_en||"")}</small></>:<span>{item.state==="error"?item.error:item.state==="ready"?"Prête à analyser":"Analyse en cours…"}</span>}</div></article>)}</div><div className="cms-ai-actions"><button className="cms-secondary" disabled={working||unique.length===0} onClick={()=>void analyzeAll()}>Analyser avec l’IA</button><button className="cms-primary" disabled={working||ready===0} onClick={()=>void createAll()}>Créer les produits regroupés</button></div></>}</section>
}
