  ...
  for(let i=0;i<catalogProducts.length;i+=batchSize){
   ...
   const rows=await Promise.all(batch.map(async p=>{
    ...
    if(candidateEvidence){
      ...
      let reason=duplicateEvidence?(visualCopy?"Image très ressemblante + détails distinctifs concordants":"Même marque + détails distinctifs concordants + image très ressemblante"):(pixel>=.72?"Image du catalogue visuellement proche — à vérifier":visual>=.68?"Ressemblance visuelle à vérifier":"Nom / marque proches — à vérifier");
