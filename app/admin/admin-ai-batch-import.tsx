...
function regroup(items:Item[]){const done=items.filter(x=>x.state==="done"&&x.suggestion&&!x.duplicate);const assigned=new Set<string>();const groups=new Map<string,string>();for(const a of done){if(assigned.has(a.id))continue;const gid=a.id;groups.set(a.id,gid);assigned.add(a.id);for(const b of done){if(assigned.has(b.id)||a.id===b.id)continue;const textScore=productSimilarity(a.suggestion!,b.suggestion!);const visual=visualSimilarity(a.visualHash,b.visualHash);const sameBrand=!!(norm(a.suggestion!.brand)&&norm(a.suggestion!.brand)===norm(b.suggestion!.brand));const sameCategory=!!(norm(a.suggestion!.category)&&norm(a.suggestion!.category)===norm(b.suggestion!.category));const aDistinct=usefulWords(`${a.suggestion!.name_fr||""} ${a.suggestion!.name_en||""}`),bDistinct=usefulWords(`${b.suggestion!.name_fr||""} ${b.suggestion!.name_en||""}`);const distinctive=overlap(aDistinct,bDistinct);const sameArticle=!!(norm(a.suggestion!.article_number)&&norm(a.suggestion!.article_number)===norm(b.suggestion!.article_number));/* Ne jamais assembler automatiquement deux produits uniquement parce qu'ils partagent marque, catégorie et emballage. Les photos d'un même article doivent avoir un identifiant commun ou plusieurs détails distinctifs concordants. */const sameProduct=sameArticle||(sameBrand&&sameCategory&&textScore>=.94&&visual>=.97&&distinctive>=.72);if(sameProduct){groups.set(b.id,gid);assigned.add(b.id)}}}return items.map(x=>groups.has(x.id)?{...x,group:groups.get(x.id)}:x)}
...
const suggestion=result.suggestion as Row;const exactMatch=await exactCatalogMatch(item.file);const matches=await rankCatalogVisually(item,suggestion);if(exactMatch&&!matches.some(m=>String(m.product.id)===String(exactMatch.id)))matches.unshift({product:exactMatch,score:1,kind:"certain",reason:"Image identique au catalogue (comparaison des pixels)"});else if(exactMatch)matches.sort((a,b)=>Number(String(b.product.id)===String(exactMatch.id))-Number(String(a.product.id)===String(exactMatch.id)));const match=exactMatch?matches.find(m=>String(m.product.id)===String(exactMatch.id)):matches[0];const suggestionWithVisibility={...suggestion,visible_qc:targetVisibility==="qc"||targetVisibility==="both",visible_conakry:targetVisibility==="conakry"||targetVisibility==="both"};const done={...item,url,suggestion:suggestionWithVisibility,group:groupKey(suggestionWithVisibility),catalogMatch:match?.product,catalogScore:match?.score,catalogMatches:matches,state:"done" as const};setItems(a=>regroup(a.map(x=>x.id===item.id?done:x)))}catch(error){setItems(a=>a.map(x=>x.id===item.id?{...x,state:"error",error:error instanceof Error?error.message:"Erreur"}:x))}finally{completed++;setAiProgress({done:completed,total:queue.length,current:item.file.name})}}setWorking(false);setAiProgress(p=>({...p,current:"Terminé"}))}
 ...
 async function retryAnalysis(item:Item){
  ...
  try{
    ...
    if(!result?.suggestion)throw(lastError instanceof Error?lastError:new Error("Analyse IA impossible après 3 tentatives"));
    const suggestion=result.suggestion as Row;const matches=await rankCatalogVisually({...item,url},suggestion);const match=exactMatch?matches.find(m=>String(m.product.id)===String(exactMatch.id)):matches[0];
 ...
 const eligible=items.filter(x=>!x.duplicate&&x.state==="done"&&x.suggestion&&x.url&&!x.error&&!x.matchAccepted&&(!x.catalogMatch||x.matchRejected));
