...
export function AiBatchImport({market,busy,onDone,catalogProducts,search,setSearch,synchronize,add}:{market:Market;busy:boolean;onDone:()=>Promise<void>;catalogProducts:Row[];search:string;setSearch:(value:string)=>void;synchronize:()=>void;add:()=>void}){
 ...
 return <section className="cms-ai-batch">
  <div style={{position:"fixed",top:10,left:218,right:24,zIndex:9990,display:"flex",alignItems:"center",gap:10,padding:"10px 12px",border:"1px solid #dce6eb",borderRadius:12,background:"#fff",boxShadow:"0 6px 18px rgba(18,52,77,.14)",overflowX:"auto",whiteSpace:"nowrap"}}>
    ...
    {items.length>0&&<><button type="button" className="cms-danger" disabled={working} onClick={cancelBatch}>Annuler</button><button className="cms-secondary" disabled={working||unique.length===0} onClick={()=>void analyzeAll()}>Analyser avec l’IA</button><button className="cms-primary" disabled={working||ready===0} onClick={()=>void createAll()}>{standby?"Enregistrer les fiches en attente":"Enregistrer et afficher les produits"}</button></>}
  </div>
  ...
  {items.length>0&&<>
    ...
    <div style={{display:"flex",gap:10,alignItems:"center",marginTop:16,flexWrap:"wrap"}}><button type="button" className="cms-secondary" disabled={working||items.length===0} onClick={()=>setSelectedImportItems(new Set(items.map(i=>i.id)))}>Tout sélectionner</button><button type="button" className="cms-secondary" disabled={working||selectedImportItems.size===0} onClick={()=>setSelectedImportItems(new Set())}>Tout désélectionner</button><button type="button" className="cms-danger" disabled={working||selectedImportItems.size===0} onClick={()=>{if(!window.confirm(`Supprimer ${selectedImportItems.size} élément(s) sélectionné(s) de cette analyse ?`))return;setItems(a=>{for(const x of a)if(selectedImportItems.has(x.id)&&x.preview)URL.revokeObjectURL(x.preview);return a.filter(x=>!selectedImportItems.has(x.id))});setSelectedImportItems(new Set())}}>🗑 Supprimer la sélection ({selectedImportItems.size})</button></div><div style={{display:"grid",gap:24,marginTop:18}}>
      {items.map(item=><article key={item.id} className={item.duplicate?"is-duplicate":""} style={{position:"relative",display:"grid",gridTemplateColumns:"420px minmax(0,1fr)",gap:24,padding:18,border:selectedImportItems.has(item.id)?"3px solid #2676a8":"1px solid #dce6eb",borderRadius:12,background:"#fff"}}><label style={{position:"absolute",zIndex:4,left:28,top:28,display:"flex",gap:7,alignItems:"center",padding:"7px 9px",background:"rgba(255,255,255,.95)",borderRadius:8,fontWeight:700}}><input type="checkbox" checked={selectedImportItems.has(item.id)} onChange={e=>setSelectedImportItems(prev=>{const next=new Set(prev);e.target.checked?next.add(item.id):next.delete(item.id);return next})}/> Sélectionner</label>
        ...
        <div style={{display:"grid",alignContent:"start",gap:10,minWidth:0}}>
          ...
          {item.duplicate?<span>{item.duplicateKind==="exact"?"Doublon exact":"Doublon visuel probable"} — ignoré</span>:item.suggestion?<>
            ...
            <label><strong>Catégorie</strong><select value={String(item.suggestion.category||"eveil")} onChange={e=>update(item.id,"category",e.target.value)}>{Object.entries(categories).map(([v,l])=><option value={v} key={v}>{l}</option>)}</select></label><div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",border:"1px solid #d9e3e8",borderRadius:9,background:"#f8fafb",maxWidth:680,flexWrap:"wrap"}}><span style={{fontWeight:700}}>Visibilité</span><select value={String(item.suggestion.category||"")==="masque"?"hidden":String(item.suggestion.visible_qc)=="true"&&String(item.suggestion.visible_conakry)=="true"?"both":String(item.suggestion.visible_qc)=="true"?"qc":String(item.suggestion.visible_conakry)=="true"?"conakry":"hidden"} onChange={e=>{const v=e.target.value;setItems(a=>a.map(x=>x.id===item.id&&x.suggestion?{...x,suggestion:{...x.suggestion,category:v==="hidden"?"masque":String(x.suggestion.category||"")==="masque"?"eveil":x.suggestion.category,visible_qc:v==="qc"||v==="both",visible_conakry:v==="conakry"||v==="both"}}:x))}} style={{padding:"7px 10px",border:"1px solid #b9cbd5",borderRadius:8,fontWeight:700}}><option value="hidden">Masqué partout</option><option value="qc">Québec seulement</option><option value="conakry">Conakry seulement</option><option value="both">Québec + Conakry</option></select></div>
