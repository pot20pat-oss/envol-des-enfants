...
export default function Administration() {
  ...
  return <AdminLayout admin={admin} section={section} market={market} notice={notice} error={error} notificationCount={notificationCount} onSection={(next)=>{changeSection(next);setError("")}} onMarket={setMarket} signOut={()=>void signOut()}>
    ...
    {section==="dashboard"&&<DashboardSection stats={stats} products={regionalProducts} orders={orders} market={market} goTo={changeSection}/>}
    ...
    {section==="settings"&&<SettingsSection market={market} settings={settings} setSettings={setSettings} passwords={passwords} setPasswords={setPasswords} busy={busy} saveSettings={(event)=>void saveSettings(event,settings)} changePassword={(event)=>void changePassword(event)}/>}
