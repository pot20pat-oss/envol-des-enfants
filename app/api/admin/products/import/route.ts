...
export async function POST(request: Request) {
  ...
  for (const candidate of data.products.slice(0, 250)) {
    ...
    statements.push(database.prepare("INSERT INTO products (id,name_fr,name_en,description_fr,description_en,category,price,stock,status,badge,ages,image_url,image_sheet,image_position,brand,material,dimensions,exchange_terms_fr,exchange_terms_en,visible,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
      .bind(crypto.randomUUID(), frenchName, stringValue(name.en), stringValue(detail.fr), stringValue(detail.en), stringValue(product.category, "eveil"), numberValue(product.price), product.status === "sold" ? 0 : 1, stringValue(product.status, "available"), stringValue(product.badge) || null, stringValue(product.ages, "3+"), stringValue(product.imageUrl) || null, stringValue(product.sheet) || null, numberValue(product.position), null, null, null, null, null, 1, now, now));
  }
