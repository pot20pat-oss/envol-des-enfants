...
type Props = {
  ...
  accent: string;
  description: string;
  onActiveChange: (category: string) => void;
...
export default function StorefrontCatalog({
  ...
  const groups = [
    { key: "jouets", label: say("Jouets", "Toys"), values: ["eveil", "vehicules", "piscine", "imitation", "dinosaures", "animaux"] },
    { key: "poupees", label: say("Mon monde de poupées et princesses", "My world of dolls and princesses"), values: dollCategories },
    { key: "ecole", label: say("École", "School"), values: ["scolaire", "sacs"] },
    { key: "enfants", label: say("Bébé & enfants", "Baby & kids"), values: ["bebe", "vetements", "chaussures"] },
  ];

  return (
    <section className="univers section wrap" id="catalogue" style={style}>
      <div className="section-heading">
        <div><p className="eyebrow">{say("Nos trouvailles en boutique", "Discover our favourite finds")}</p><h2>{title}<br /><span>{accent}</span></h2></div>
        <p>{description}</p>
      </div>
      ...
      <div className="catalog-search">
        ...
        <select value={status} onChange={(event) => onStatusChange(event.target.value)} aria-label={say("Filtrer par disponibilité", "Filter by availability")}>
          <option value="all">{say("Tous les statuts", "All availability")}</option><option value="available">{say("Disponible", "Available")}</option><option value="reserved">{say("Réservé", "Reserved")}</option><option value="sold">{say("Vendu", "Sold out")}</option>
        </select>
      </div>
          ...
          return (
            <details className={`category-menu-group category-menu-${group.key}${groupActive ? " is-active" : ""}`} key={group.key}>
              ...
              <div className="category-menu-panel">
                ...
                {children.filter((category) => !(group.key === "poupees" && category.value === "poupees")).map((category) => (
                  <button type="button" key={category.value} className={`${group.key === "poupees" ? "doll-brand-choice" : ""}${active === category.value ? " active" : ""}`} onClick={(event) => { chooseCategory(category.value); event.currentTarget.closest("details")?.removeAttribute("open"); }}>{category.label[language].replace("↳ ", "")}</button>
                ))}
              </div>
            </details>
          );
      ...
      <div className="product-grid" id="coups-de-coeur" key={`${active}-${status}-${query}`}>
        {visibleProducts.map((item) => <ProductCard key={item.id || `${item.sheet}-${item.position}`} item={item} language={language} market={market} whatsappNumber={whatsappNumber} whatsappUrl={whatsappUrl} onOpen={onOpenProduct} />)}
      </div>
