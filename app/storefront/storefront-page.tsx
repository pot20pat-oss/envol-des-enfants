  ...
  return (
    <main className="editable-storefront">
      ...
      <StorefrontFeaturedCollections
        ...
        market={market}
        say={say}
        onSelectProduct={(search) => searchCatalog(search)}
      />
      ...
      <section className="brands-section"><div className="wrap"><p className="eyebrow">{say("Des marques que les enfants adorent", "Brands little ones love")}</p><div className="brands-line">{["Crayola","Disney","LEGO","Hype","Mattel","Fisher-Price","Hasbro"].map((brand) => <a href="#catalogue" key={brand} onClick={() => searchCatalog(brand)}>{brand}</a>)}</div></div></section>
